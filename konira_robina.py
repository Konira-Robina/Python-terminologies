...
# it is like giving a recipe for food or instructions on how aparticular food can be cooked. If the instructions are clear, the food comes out right
#example:
print(konra) 
"""
this statement tells the computer to display the message "konra" on the screen.
it shows how programming is giving instructions to the computer.
""""
# 2. memory partitioning
# memory partitioning means dividing memory into sections.
...
"""
# 3. memory address
# memory address is the exact location where data is stored.
# it is like a house address that helps you find a specific home.
# example 
x = 5
print(id(x))
"""
the variable x stores the values 5
the id(x) shows the memory address where 5 is stored
this proves that every value has a unique location in memory.
"""
# 4. how RAM connects the CPU and the hard disk
# RAM helps the CPU quickly access data from the hard disk.
...
it shows how multiple statements work together
"""
...
# syntax is like grammer in English or the rules that tell us about how English should be spoken.
...
# rule 4
# #avoid using capital letters while writing varaiable names eg NAME, AGE
